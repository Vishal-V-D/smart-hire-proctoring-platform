...
const aspect = 1.2;
// UPDATED: Brought camera closer (from 300 to 180) to make globe big without CSS scaling
const cameraZ = 250;
...
export function Globe({ globeConfig, data }: WorldProps) {
  ...
  useEffect(() => {
    ...
    globeRef.current
      .pointsData(data) // Simplified using data as points for demo safety
  ...
  // Rings animation loop
  useEffect(() => {
    if (!globeRef.current || !isInitialized || !data) return;
    const interval = setInterval(() => {
      if (!globeRef.current) return;
      const ringsData = data.filter((d, i) => i % 2 === 0).map((d) => ({ lat: d.startLat, lng: d.startLng, color: d.color }));
      globeRef.current.ringsData(ringsData);
    }, 2000);
    return () => clearInterval(interval);
  }, [isInitialized, data]);

  // UPDATED: Strictly position group at 0,0,0
...
export function World(props: WorldProps) {
  ...
  return (
    <Canvas scene={scene} camera={{ position: [0, 0, cameraZ], fov: 50 }}>
      ...
      <OrbitControls
        ...
        // UPDATED: Ensure it looks at the absolute center
        target={[0, 0, 0]}
      />
    </Canvas>
  );
