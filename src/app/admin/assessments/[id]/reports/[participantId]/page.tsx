...
export default function AdminParticipantReportPage() {
                    ...
                    {isLandscape ? (
                        ...
                        <>
                                        ...
                                        {report.codingProblems.map((problem, idx) => (
                                            <div key={idx} className="flex items-center justify-between p-2 bg-gray-50 rounded border border-gray-200 text-xs">
                                                <div className="flex-1 min-w-0">
                                                    <p className="font-medium truncate">{problem.problemTitle}</p>
                                                    <p className="text-[10px] text-gray-500">Lang: {problem.language}</p>
                                                </div>
                                                <div className="flex items-center gap-3 ml-2">
                                                    <span className="text-[10px] text-gray-500">
                                                        {problem.passedTests}/{problem.totalTests}
                                                    </span>
                                                    <span className="font-bold">
                                                        {problem.score}/{problem.maxScore}
                                                    </span>
                                                    <span className={`text-[10px] px-1.5 py-0.5 rounded capitalize ${getCodingStatusColor(problem.status)}`}>
                            ...
                            <div className="pt-2 border-t border-gray-300 text-center text-[10px] text-gray-400">
                                <p>Computer-generated report • {new Date().toLocaleString('en-IN')} • Confidential</p>
                            </div>
                        </>
                    ) : (
                                    ...
                                    {report.session.systemChecks && (
                                        <div className="flex items-center gap-4 mt-3 text-xs text-gray-500">
                                            <span className="flex items-center gap-1">
                                                <Monitor size={12} /> Browser: {report.session.systemChecks.browser ? '✓' : '✗'}
                                            </span>
                                            <span className="flex items-center gap-1">
                                                <Camera size={12} /> Camera: {report.session.systemChecks.camera ? '✓' : '✗'}
                                            </span>
                                            <span className="flex items-center gap-1">
                                                <Mic size={12} /> Mic: {report.session.systemChecks.mic ? '✓' : '✗'}
                                            </span>
                                        </div>
                                    )}
                                </div>
